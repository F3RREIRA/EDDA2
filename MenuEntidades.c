...
/* Função particionar o vetorRegistro */
int partition(registro_osc *vetorRegistro, int start, int pivot) 
{
	registro_osc aux;
	int i; 
...
/* Função para ordenar o vetorRegistro */
int quickSort(registro_osc *vetorRegistro, int start, int end) 
{
	int pivot;
	if (start < end) {
...
void OrdenaCNPJDAT(void)
{
   ...
   quickSort(vet, 0, ind-1) ;
	...
	printf ("\nO arquivo 'OSC.DAT' tem %ld bytes\n\n", findSize("OSC.DAT") );
	printf ("Tecle [Enter] para voltar ao MENU ENTIDADES..."); 
	getch();
...
int MenuEntidades (void){ 
    ...
    printf ("----------------------------------------------------------\n\n");
	printf ("          << MENU ENTIDADES DA SOCIEDADE CIVIL >>\n\n"); 
